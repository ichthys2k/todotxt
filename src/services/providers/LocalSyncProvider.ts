...
declare global {
  interface Window {
    electronAPI?: {
      ...
      onFileChanged: (callback: (type: 'todo' | 'archive') => void) => () => void;
    };
  }
}
...
export const requestFileHandlePermission = async (type: 'todo' | 'archive'): Promise<boolean> => {
  if (Capacitor.isNativePlatform()) {
    return await ensureCapacitorPermissions();
  }
...
export class LocalSyncProvider implements SyncProvider {
  async fetchTodoContent(): Promise<string> {
    if (Capacitor.isNativePlatform()) {
      ...
      if (!uriStr) {
        throw new Error('FILE_HANDLE_LOST');
      }
      try {
        const res = await SafStorage.readFile({
          folderUri: uriStr,
          fileName: 'todo.txt'
        });
        ...
        console.error('SafStorage: Failed to read todo.txt', e);
  ...
  async fetchArchiveContent(): Promise<string> {
    if (Capacitor.isNativePlatform()) {
      const uriStr = localStorage.getItem('todo_txt_saf_folder_uri');
      if (!uriStr) {
        throw new Error('ARCHIVE_FILE_HANDLE_LOST');
      }
      try {
        const res = await SafStorage.readFile({
          folderUri: uriStr,
        ...
        console.error('SafStorage: Failed to read archive.txt', e);
  ...
  async fetchConfigContent(): Promise<string> {
    if (Capacitor.isNativePlatform()) {
      ...
      try {
        const res = await SafStorage.readFile({
          folderUri: uriStr,
          fileName: 'todo.config.json'
        });
        const text = res.content || '{}';
        localStorage.setItem(LOCAL_CONFIG_KEY, text);
        return text;
      } catch (e: any) {
        console.warn('SafStorage: Failed to read config.json', e);
    ...
    if (Capacitor.isNativePlatform()) {
      ...
      if (uriStr) {
        try {
          ...
          console.error('SafStorage: Failed to write config', e);
        }
      }
    }
  ...
  async getTodoLastModified(): Promise<number> {
    if (Capacitor.isNativePlatform() || window.electronAPI) {
      return 0;
    }
    const handle = await getTodoFileHandle();
    if (handle) {
      ...
      if (hasPerm) {
        try {
          ...
          console.error('Failed to get lastModified for todo handle', e);
        }
      }
    }
    return 0;
  ...
  async getArchiveLastModified(): Promise<number> {
    if (Capacitor.isNativePlatform() || window.electronAPI) {
      return 0;
    }
    const handle = await getArchiveFileHandle();
    if (handle) {
      ...
      if (hasPerm) {
        try {
          ...
          console.error('Failed to get lastModified for archive handle', e);
        }
      }
    }
    return 0;
