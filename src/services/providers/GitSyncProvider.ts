  ...
  private async saveFile(path: string, content: string, shaKey: string, cacheKey: string, pendingKey: string): Promise<string> {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Conflict! Jemand anderes hat gepusht.
        console.warn(`Konflikt bei ${path} erkannt. Starte automatischen Merge...`);
        
        // 1. Neueste Version von GitHub herunterladen
        const getRes = await githubApiCall(path, 'GET');
        if (getRes.ok) {
          const data = await getRes.json();
          const remoteContent = decodeBase64(data.content);
          const remoteSha = data.sha;
          
          // 2. Zusammenführen (Merge)
          let mergedContent = '';
          if (path.endsWith('.json')) {
            const { mergeConfigContents } = await import('../../utils/todoMerger');
            mergedContent = mergeConfigContents(content, remoteContent);
          } else {
            const { mergeTodoContents } = await import('../../utils/todoMerger');
            mergedContent = mergeTodoContents(content, remoteContent);
          }
          
          // 3. Cache und SHA aktualisieren
          localStorage.setItem(cacheKey, mergedContent);
          localStorage.setItem(shaKey, remoteSha);
          ...
          if (retryRes.ok) {
            ...
            localStorage.removeItem(pendingKey);
            console.log(`Automatische Zusammenführung für ${path} erfolgreich abgeschlossen.`);
            return mergedContent;
          } else {
            throw new Error(`Zusammenführung fehlgeschlagen (Fehler nach Merge-Versuch: ${retryRes.status})`);
          }
        } else {
          throw new Error(`Zusammenführung fehlgeschlagen (Konnte remote Version nicht laden: ${getRes.status})`);
        }
      } else {
        throw new Error(`GitHub API Error: ${res.status}`);
      }
