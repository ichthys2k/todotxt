...
import { ProfileSyncManagerUI } from './ProfileSyncManagerUI';
import { KanbanBoard } from './KanbanBoard';
import { DashboardView } from './DashboardView';
import { RawTextEditor } from './RawTextEditor';
import { Archive, Wifi, RefreshCw, Rows, LayoutGrid, Menu, Undo, ArrowUpDown, Settings, Filter, HelpCircle, Layers, CheckCircle, Sliders, Palette, Database, Smile, Globe, Share2, Merge, Copy, X, AlertTriangle, Download } from 'lucide-react';
...
import { getCoreTaskText } from '../utils/todoMerger';
...
const migrationTranslations: Record<'de' | 'en', Record<string, string>> = {
  de: {
    ...
    mergeOptionTitle: 'Zusammenführen (Merge)',
    mergeOptionDesc: 'Nimm die neue Datei als Maßstab und vergleiche die alte Datei mit ihr: Falls es zusätzliche Aufgaben gibt, füge sie der neuen Datei hinzu und gib ihnen das Projekt "+import".',
    overwriteOptionTitle: 'Alte Aufgaben überschreiben neue Datei / Dienst',
    overwriteOptionDesc: 'Ersetzt alle Aufgaben im neuen Speicher komplett mit deinen vorhandenen alten Aufgaben.',
    downloadOptionTitle: 'Alte Aufgaben löschen und mit neuer Datei / Dienst überschreiben',
    downloadOptionDesc: 'Löscht die alten Aufgaben in der App und lädt die Aufgaben aus dem neuen Speicher herunter.',
  ...
  en: {
    ...
    mergeOptionTitle: 'Merge Tasks',
    mergeOptionDesc: 'Use the new file as a baseline and compare the old file with it. Additional tasks are added to the new file with the project "+import".',
    overwriteOptionTitle: 'Overwrite new file / service with old tasks',
    overwriteOptionDesc: 'Replaces all tasks in the new storage completely with your existing old tasks.',
    downloadOptionTitle: 'Delete old tasks and overwrite with new file / service',
    downloadOptionDesc: 'Deletes the old tasks in the app and downloads the tasks from the new storage.',
...
export const TodoApp = ({ storageMode, onLogout, onSetupSync, username: _username, avatarUrl: _avatarUrl }: TodoAppProps) => {
  ...
  const handlePerformMigration = async (type: 'merge' | 'overwrite' | 'download') => {
    ...
    try {
      ...
      if (type === 'download') {
        ...
        const currentTasks = parseTodos(currentTodo);
        const prevTasks = parseTodos(migrationData.prevTasksStr);
        
        const currentCores = new Set(currentTasks.map(t => getCoreTaskText(t.originalText)));
        const mergedTasks = [...currentTasks];
        
        for (const prevTask of prevTasks) {
          const prevCore = getCoreTaskText(prevTask.originalText);
          if (!currentCores.has(prevCore)) {
            const updatedTask = { ...prevTask };
            if (!updatedTask.projects.includes('import')) {
              updatedTask.projects = [...updatedTask.projects, 'import'];
              const trimmedDesc = updatedTask.description.trim();
              updatedTask.description = trimmedDesc ? `${trimmedDesc} +import` : '+import';
            }
        ...
        finalTodo = serializeTodos(mergedTasks);

        const currentArchiveTasks = parseTodos(currentArchive);
        const prevArchiveTasks = parseTodos(migrationData.prevArchiveStr);
        const mergedArchiveTasks = [...currentArchiveTasks];
        const currentArchiveCores = new Set(currentArchiveTasks.map(t => getCoreTaskText(t.originalText)));
        
        for (const prevTask of prevArchiveTasks) {
          const prevCore = getCoreTaskText(prevTask.originalText);
          if (!currentArchiveCores.has(prevCore)) {
            mergedArchiveTasks.push(prevTask);
          }
        }
        finalArchive = serializeTodos(mergedArchiveTasks);
  ...
  const [currentView, setCurrentView] = useState<'todo' | 'archive' | 'dashboard' | 'raw-editor'>(() => {
    const params = new URLSearchParams(window.location.search);
    const viewParam = params.get('view');
    if (viewParam === 'dashboard') return 'dashboard';
    if (viewParam === 'archive') return 'archive';
    if (viewParam === 'raw-editor') return 'raw-editor';
    return 'todo';
  });
  ...
  // Undo history state
  const [history, setHistory] = useState<{ tasks: TodoTask[]; archivedTasks: TodoTask[]; view: 'todo' | 'archive' | 'dashboard' | 'raw-editor' }[]>([]);

  const pushToHistory = (currentTasks: TodoTask[], currentArchivedTasks: TodoTask[], view: 'todo' | 'archive' | 'dashboard' | 'raw-editor') => {
    setHistory(prev => {
  ...
  // Experimentelle Sync-Features (Standard: aus)
  ...
  useEffect(() => {
    localStorage.setItem('todo_txt_enable_experimental_sync', enableExperimentalSync ? 'true' : 'false');
  }, [enableExperimentalSync]);
  ...
  const [showMetadataTags, setShowMetadataTags] = useState<boolean>(() => {
    return localStorage.getItem('todo_txt_show_metadata_tags') === 'true';
  });

  useEffect(() => {
    localStorage.setItem('todo_txt_show_metadata_tags', showMetadataTags ? 'true' : 'false');
  }, [showMetadataTags]);
  ...
  const handleResetAllData = async () => {
    if (window.confirm(t('resetDataConfirm', language))) {
      ...
      // Delete native Capacitor files if on native platform
      if (Capacitor.isNativePlatform()) {
        try {
          ...
          console.warn('Failed to delete todo.txt from Data:', e);
        }
        ...
        try {
          ...
          console.warn('Failed to delete archive.txt from Data:', e);
        }
        ...
        try {
          ...
          console.warn('Failed to delete todo.config.json from Data:', e);
        }

        try {
          await Filesystem.deleteFile({
            path: 'localstorage_backup.json',
            directory: Directory.Data
          });
        } catch (e) {
          console.warn('Failed to delete localstorage_backup.json from Data:', e);
        }
      }
      ...
      if (configStr && configStr.trim() !== '{}' && configStr.trim() !== '') {
        ...
        if (config) {
          ...
          if (config.settings) {
            ...
            if (typeof s.showMetadataTags === 'boolean') setShowMetadataTags(s.showMetadataTags);
            ...
            if (typeof s.enableExperimentalSync === 'boolean') setEnableExperimentalSync(s.enableExperimentalSync);
  ...
  const hasLocalFileSelected = () => {
    if (Capacitor.isNativePlatform()) {
      return !!localStorage.getItem('todo_txt_saf_folder_uri');
    }
    return localStorage.getItem('todo_txt_local_setup_complete') === 'true';
  };
  ...
  // Automatischer Sync/Reload bei Tab-Fokus und Electron-Watcher / Web-Polling
  useEffect(() => {
    const handleFocus = () => {
      if (isLocalMode && hasFileSelected) {
        loadTasks(true);
      } else if (isOnline && !isLocalMode && hasFileSelected && !syncing) {
        handleSync();
      }
    };
    window.addEventListener('focus', handleFocus);
    return () => window.removeEventListener('focus', handleFocus);
  }, [isOnline, isLocalMode, hasFileSelected, syncing, tasks, currentView]);

  // Electron file watcher subscription
  useEffect(() => {
    if (isElectron && (window as any).electronAPI?.onFileChanged) {
      const cleanup = (window as any).electronAPI.onFileChanged((type: 'todo' | 'archive') => {
        if (
          (type === 'todo' && currentView === 'todo') ||
          (type === 'archive' && currentView === 'archive')
        ) {
          loadTasks(true);
        }
      });
  ...
  }, [currentView, isElectron, tasks]);

  // Web-only file metadata polling for changes (FSA API)
  useEffect(() => {
    if (!isLocalMode || isElectron || !hasFileSelected) return;

    let lastModTime = 0;

    const checkTimestamp = async () => {
      try {
        if (Date.now() - lastSaveTimestampRef.current < 8000) {
          return; // Skip auto-reload immediately after saving
        }
        const modTime = currentView === 'todo'
          ? await getTodoLastModified(storageMode)
          : await getArchiveLastModified(storageMode);
        
        if (modTime > 0) {
          if (lastModTime > 0 && modTime !== lastModTime) {
            loadTasks(true);
          }
          lastModTime = modTime;
        }
      } catch (e) {
        console.error("Failed to check metadata timestamp:", e);
      }
    };
    ...
    return () => clearInterval(interval);
  }, [isLocalMode, isElectron, hasFileSelected, currentView, storageMode, tasks]);
  ...
  const loadTasks = async (silent = false) => {
    try {
      ...
      const content = (currentView === 'todo' || currentView === 'raw-editor') 
        ? await fetchTodoContent(storageMode)
        : await fetchArchiveContent(storageMode);

      // Compare content with currently loaded tasks to avoid redundant updates:
      if (silent && tasks.length > 0) {
        const currentSerialized = serializeTodos(tasks);
        const normalize = (s: string) => s.replace(/\r\n/g, '\n').trim();
        if (normalize(content) === normalize(currentSerialized)) {
          return;
        }
      }
      ...
      } else if (err.message === 'FILE_HANDLE_LOST' || err.message === 'ARCHIVE_FILE_HANDLE_LOST') {
        setError(err.message);
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-200">
                ...
                <div className="flex md:hidden items-center gap-2 flex-shrink-0">
                  <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-full font-semibold border bg-slate-50 dark:bg-slate-950 border-slate-200 dark:border-slate-850">
                    <button 
                      ...
                      className="flex items-center gap-1.5 text-slate-800 dark:text-slate-200 hover:text-indigo-650 dark:hover:text-indigo-400 disabled:opacity-50 transition-colors cursor-pointer border-none bg-transparent p-0 font-semibold"
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        <div className={`flex-1 flex flex-col ${(layoutMode === 'kanban' && currentView === 'todo') || currentView === 'raw-editor' ? 'max-w-none px-4 md:px-6 overflow-hidden' : 'max-w-4xl mx-auto overflow-y-auto'} w-full p-4 md:p-6 relative`}>
            ...
            <div className="mb-4 p-4 rounded-lg bg-red-100 dark:bg-red-900/50 border border-red-200 dark:border-red-500/50 text-red-700 dark:text-red-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs sm:text-sm font-sans">
              <span className="font-medium">
                {error === 'FILE_PERMISSION_REQUIRED' || error === 'ARCHIVE_FILE_PERMISSION_REQUIRED'
                  ...
                  : error === 'FILE_HANDLE_LOST' || error === 'ARCHIVE_FILE_HANDLE_LOST'
                  ? 'Der Browser hat die Verknüpfung zur lokalen Datei verloren. Bitte wähle die Datei erneut aus.'
                  : error}
              </span>
              {(error === 'FILE_PERMISSION_REQUIRED' || error === 'ARCHIVE_FILE_PERMISSION_REQUIRED') && (
                <button
              ...
              {(error === 'FILE_HANDLE_LOST' || error === 'ARCHIVE_FILE_HANDLE_LOST') && (
                <button
                  ...
                  className="px-3.5 py-1.5 bg-red-600 hover:bg-red-700 text-white rounded-lg text-xs font-semibold shadow-sm cursor-pointer transition-colors"
                >
                  Datei erneut verknüpfen
                </button>
              )}
            </div>
          )}
          ...
          <div className={`${layoutMode === 'kanban' && currentView === 'todo' ? 'mt-2 mb-2' : 'mt-2 mb-4'} flex justify-between items-center flex-shrink-0`}>
            <div className="flex items-center gap-3">
              <h2 className="text-xl font-bold">
                {currentView === 'todo' ? t('tasksTitle', language) : currentView === 'archive' ? t('archiveTitle', language) : currentView === 'raw-editor' ? (language === 'de' ? 'Direkt-Editor' : 'Raw Editor') : t('dashboardTitle', language)}
              </h2>
            ...
            {currentView !== 'dashboard' && currentView !== 'raw-editor' && (
          ...
          <div className={`transition-opacity ${loading ? 'opacity-50 pointer-events-none' : ''} ${(layoutMode === 'kanban' && currentView === 'todo') || currentView === 'raw-editor' ? 'flex-1 min-h-0 flex flex-col' : 'h-full'}`}>
            {currentView === 'dashboard' ? (
              ...
              />
               ...
               <TodoList 
                ...
                storageMode={storageMode}
                isLocalFileLinked={isLocalFileLinked}
                showMetadataTags={showMetadataTags}
              />
            )}
          </div>
                ...
                {activeSettingsTab === 'files' && (
                  <div className="space-y-5 animate-fade-in">
                    <div className="space-y-4 bg-slate-50 dark:bg-slate-950/40 p-4 rounded-2xl border border-slate-200 dark:border-slate-800/80">
                      ...
                      {enableExperimentalSync && (
                        <div className="border-t border-slate-200 dark:border-slate-800/80 pt-4 mt-2 space-y-3">
                          <span className="font-bold text-slate-750 dark:text-slate-300 text-xs">Mit Cloud-Dienst verbinden</span>
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                            ...
                            <button
                              ...
                              className="flex items-center justify-center gap-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-200 py-2.5 px-3 rounded-xl font-medium text-xs hover:bg-slate-50 dark:hover:bg-slate-800/60 cursor-pointer"
                            >
                            ...
                            <button
                              onClick={() => {
                                ...
                                localStorage.removeItem('todo_txt_webdav_path');
                                window.location.reload();
                              }}
                              className="flex items-center justify-center gap-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-200 py-2.5 px-3 rounded-xl font-medium text-xs hover:bg-slate-50 dark:hover:bg-slate-800/60 cursor-pointer"
                            >
                              <Wifi size={14} className="text-indigo-500" />
                              <span>WebDAV Server</span>
                            </button>
                            ...
                            <button
                              onClick={() => {
                                ...
                                localStorage.removeItem('todo_txt_git_pat');
                                window.location.reload();
                              }}
                              className="flex items-center justify-center gap-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-200 py-2.5 px-3 rounded-xl font-medium text-xs hover:bg-slate-50 dark:hover:bg-slate-800/60 cursor-pointer"
                            >
                        ...
                        <div className="flex gap-3 pt-3">
                          ...
                          {(storageMode !== 'local' || enableExperimentalSync) && (
                            <button
                              onClick={onLogout}
                    ...
                    <div className="space-y-4 bg-slate-50 dark:bg-slate-950/40 p-4 rounded-2xl border border-slate-200 dark:border-slate-800/80">
                      <span className="text-xs font-semibold text-slate-450 dark:text-slate-500 uppercase tracking-wider flex items-center gap-1.5">
                        <AlertTriangle size={13} className="text-amber-500" /> Experimentelles Feature
                      </span>
                      <label className="flex items-center gap-3 bg-white dark:bg-slate-900/50 hover:bg-slate-100/50 dark:hover:bg-slate-850/50 p-3.5 rounded-xl border border-slate-200/40 dark:border-slate-800/40 cursor-pointer select-none transition-colors">
                        <input
                          type="checkbox"
                          checked={enableExperimentalSync}
                          onChange={(e) => setEnableExperimentalSync(e.target.checked)}
                        ...
                        <div className="flex flex-col gap-0.5">
                          <span className="text-sm font-semibold text-slate-750 dark:text-slate-200">{t('enableExperimentalSync', language)}</span>
                          <span className="text-xs text-slate-500 dark:text-slate-455">Aktiviert OneDrive, Google Drive, WebDAV und GitHub Synchronisation.</span>
                        </div>
                      </label>
                    </div>
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="border-t border-slate-205 dark:border-slate-805/80 my-3 pt-3 space-y-3">
                          {[
                            ...
                            { key: 'showMetadataTags', checked: showMetadataTags, onChange: (val: boolean) => setShowMetadataTags(val), label: t('showMetadataTags', language) },
                          ].map((cItem) => (
      ...
      {/* Help Modal */}
      <HelpModal isOpen={isHelpOpen} onClose={() => setIsHelpOpen(false)} language={language} onSetupSync={enableExperimentalSync ? onSetupSync : undefined} />
    </div>
  );
};
