...
interface LoginProps {
  ...
  onWebDavMode: () => void;
  onGitMode: () => void;
  onGoogleDriveMode: (token: string, userId?: string) => void;
  externalError?: string | null;
  enableExperimentalSync?: boolean;
}

export const Login = ({ onLocalMode, onWebDavMode, onGitMode, onGoogleDriveMode, externalError, enableExperimentalSync = false }: LoginProps) => {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const isElectron = typeof window !== 'undefined' && window.navigator.userAgent.toLowerCase().includes('electron');

  const ensureExternalConsent = () => {
    try {
      const savedConsent = localStorage.getItem('todo_txt_cookie_consent');
      const parsed = savedConsent ? JSON.parse(savedConsent) : { essential: true, externalServices: false };
      if (!parsed.externalServices) {
        parsed.externalServices = true;
        localStorage.setItem('todo_txt_cookie_consent', JSON.stringify(parsed));
      }
    } catch (e) {
      localStorage.setItem('todo_txt_cookie_consent', JSON.stringify({ essential: true, externalServices: true }));
    }
  };
  ...
  const handleGoogleLogin = () => {
    setLoading(true);
    setError(null);
    ensureExternalConsent();
    googleAuthService.login(
      (user) => {
        setLoading(false);
        // Pass token and userId to enable encryption
        onGoogleDriveMode(user.token, user.id);
      },
      (err) => {
        ...
        <div className="space-y-4">
          {enableExperimentalSync ? (
            <>
              ...
              <div id="google-signin-btn-container" className="mt-3 flex justify-center"></div>
            ...
            <button 
              onClick={onLocalMode}
              className="w-full flex items-center justify-center gap-2.5 bg-indigo-650 hover:bg-indigo-700 dark:bg-indigo-600 dark:hover:bg-indigo-700 text-white py-4 px-5 rounded-xl font-semibold transition-all shadow-md hover:shadow-indigo-500/20 active:scale-98 cursor-pointer border-none"
            >
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="w-5 h-5">
                <path d="M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2 2 6.477 2 12s4.477 10 10 10z"/>
                <path d="M12 8v8M8 12h8"/>
              </svg>
              <span>Lokale todo.txt Datei verknüpfen</span>
            </button>
          )}
        </div>
